...
impl Dir {
    fn new(name: &str, parent: Option<Rc<Dir>>) -> Dir {
        Dir { 
            name: name.to_string(), 
    ...
    fn total_size(&self, limit: i32) -> (i32, i32) {
        let size : i32 = self.files.borrow().iter().map(|x| x.size).sum();
        let dir_sizes : Vec<(i32, i32)> = self.dirs.borrow().iter().map(|x| x.total_size(limit)).collect();
    ...
    fn max_size(&self, limit: i32) -> (i32, i32) {
        let size : i32 = self.files.borrow().iter().map(|x| x.size).sum();
...
fn solve1(data: String) -> i32 {
    let root = parse(data);
    root.total_size(100000).1
}

fn solve2(data: String) -> i32 {
    let root = parse(data);
    let total_size = root.total_size(100000).0;
    let unused = 70_000_000 - total_size;
    let need_to_free = 30_000_000 - unused;
    root.max_size(need_to_free).1
}
...
fn parse(input: String) -> Rc<Dir> {
    ...
    for line in lines {
        match line.as_slice() { 
            ...
            [size, _/*name*/] => current.files.borrow_mut().push(File { /*name: name.to_string(),*/ size: size.parse().unwrap()}),
...
mod tests {
    ...
    fn exercise2() {
        ...
        assert!(result < 3681111);
        assert_eq!(result, 2568781);
    }

fn solve1(data: String) -> String {
    ...
    //let decimal = parsed.iter().map(|l| l.enumerate(|(i, n)| n * 5^i).sum()).sum();
    let mut sum = [0; 30];
    //parsed.iter().for_each(|l| l.enumerate().for_each(|(i, n)| sum[i] += l));
    for (i, n) in parsed.iter().map(|l| l.iter().enumerate()).flatten() {
        sum[i] += n;
        let mut index = i;
        while sum[index] > 2
        {
    ...
    sum.iter().rev().map(|s| match s {
        -1 => '-',
        -2 => '=',
        x => (b'0' + *x as u8) as char
...
mod tests {
    ...
    fn exercise1() {
        ...
        assert_eq!(result, "2-0-020-1==1021=--01");
    }

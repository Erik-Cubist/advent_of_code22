enum Instruction {
    Move{d: u32},
    Left,
    Right
}

fn solve1(data: String) -> usize {
    let (map, instruction) = parse(data);
    let mut row = 0;
    let mut column = map[0].iter().position(|c| *c == '.').unwrap();
    let mut direction = 0;
    
    for next in instruction {
        match next {
            Instruction::Move { d } => { match direction {
                0 => column = move_right(&map[row], column, d, 1),
                1 => row = move_up(&map, row, column, d, 1),
                2 => column = move_right(&map[row], column, d, -1),
                3 => row = move_up(&map, row, column, d, -1),
                _ => panic!("illegal direction")
            }}
            Instruction::Left => direction = (direction + 3) % 4,
            Instruction::Right => direction = (direction + 1) % 4,
        }
    }

    (row + 1) * 1000 + (column + 1) * 4 + direction
}

fn move_right(row: &Vec<char>, column: usize, steps: u32, increment: i32) -> usize {
    let mut remaining = steps;
    let mut new_column = column;
    let mut index = column;
    while remaining > 0 {
        let new_index = get_index(index, row.len(), increment); 
        match row[new_index] {
            '.' => {remaining -= 1; new_column = new_index; }
            '#' => break,
            _  => ()
        }
        index = new_index;
    }
    
     new_column
}

fn move_up(map: &Vec<Vec<char>>, row: usize, column: usize, steps: u32, increment: i32) -> usize {
    let mut remaining = steps;
    let mut new_row = row;
    let mut index = row;
    while remaining > 0 {
        let new_index = get_index(index, map.len(), increment);
        match map[new_index].get(column) {
            Some('.') => {remaining -= 1; new_row = new_index; }
            Some('#') => break,
            _  => ()
        }
        index = new_index;
    }

    new_row
}

fn get_index(current: usize, length: usize, direction: i32) -> usize {
    let next = (current as i32 + direction) % length as i32;
    if next < 0 {
        (next + length as i32) as usize
    } else {
        next as usize
    }
}

fn solve2(data: String) -> usize {
    let (mut map, instruction) = parse(data);
    map.insert(0, Vec::new());
    for line in map.iter_mut() {
        line.insert(0, ' ');
    }
    let mut direction = 0;
    let mut row = 1;
    let mut column = map[1].iter().position(|c| *c == '.').unwrap();

    for next in instruction {
        match next {
            Instruction::Move { d } => { match direction {
                0 => (row, column, direction) = move_wrap(&map, row, column, d, (0, 1)),
                1 => (row, column, direction) = move_wrap(&map, row, column, d, (1, 0)),
                2 => (row, column, direction) = move_wrap(&map, row, column, d, (0, -1)),
                3 => (row, column, direction) = move_wrap(&map, row, column, d, (-1, 0)),
                _ => panic!("illegal direction")
            }}
            Instruction::Left => direction = (direction + 3) % 4,
            Instruction::Right => direction = (direction + 1) % 4,
        }
    }

    row * 1000 + column * 4 + direction as usize
}

fn move_wrap(map: &Vec<Vec<char>>, row: usize, column: usize, steps: u32, increment: (i32, i32)) -> (usize, usize, i32) {
    let mut remaining = steps;
    let mut new_row = row;
    let mut new_column = column;
    let mut index_x = row;
    let mut index_y = column;
    let mut inc_x = increment.0;
    let mut inc_y = increment.1;
    while remaining > 0 {
        (index_x, index_y) = get_index_wrap(index_x, index_y, inc_x, inc_y);
        match map.get(index_x).and_then(|r| r.get(index_y)) {
            Some('.') => (),
            Some('#') => break,
            _  => {
                let (ind_x, ind_y, incr_x, incr_y) = wrap(index_x, index_y, inc_x, inc_y);
                if map[ind_x][ind_y] == '#' { break; }
                else { (index_x, index_y, inc_x, inc_y) = (ind_x, ind_y, incr_x, incr_y)}
            }
        }
        remaining -= 1;
        new_row = index_x;
        new_column = index_y;
    }

    (new_row, new_column,  get_direction(inc_x, inc_y))
}

fn get_direction(inc_x: i32, inc_y: i32) -> i32 {
    inc_x * (-1 + 2 * inc_x) + inc_y * (inc_y - 1)
}

fn wrap(pos_x: usize, pos_y: usize, step_x: i32, step_y: i32) -> (usize, usize, i32, i32) {
    match (pos_x, pos_y, step_x, step_y) {
        (9, 1..=4, 1, 0) => (16, 13 - pos_y, -1, 0),
        (9, 5..=8, 1, 0) => (17 - pos_y, 9, 0, 1),
        (13, 9..=12, 1, 0) => (8, 13 - pos_y, -1, 0),
        (13, 13..=16, 1, 0) => (21 - pos_y, 1, 0, 1),
        (1..=4, 13, 0, 1) => (13 - pos_x, 16, 0, -1),
        (5..=8, 13, 0, 1) => (9, 21 - pos_x, 1, 0),
        (9..=12, 17, 0, 1) => (13 - pos_x, 8, 0, -1),
        (1..=4, 8, 0, -1) => (5, 17 - pos_x, 1, 0),
        (5..=8, 0, 0, -1) => (16, 21 - pos_x, -1, 0),
        (9..=12, 8, 0, -1) => (8, 17 - pos_x, -1, 0),
        (4, 1..=4, -1, 0) => (1, 13 - pos_y, 1, 0),
        (4, 5..=8, -1, 0) => (pos_y - 4, 9, 0, 1),
        (0, 9..=12, -1, 0) => (4, 13 - pos_y, -1, 0),
        (8, 13..=16, -1, 0) => (21 - pos_y, 12, 0, -1),
        (201, 1..=50, 1, 0) => (1, pos_y + 100, 1, 0),
        (151, 51..=100, 1, 0) => (100 + pos_y, 50, 0, -1),
        (51, 101..=150, 1, 0) => (pos_y - 50, 100, 0, -1),
        (1..=50, 151, 0, 1) => (151 - pos_x, 100, 0, -1),
        (51..=100, 101, 0, 1) => (50, 50 + pos_x, -1, 0),
        (101..=150, 101, 0, 1) => (151 - pos_x, 150, 0, -1),
        (151..=200, 51, 0, 1) => (150, pos_x - 100, -1, 0),
        (1..=50, 50, 0, -1) => (151 - pos_x, 1, 0, 1),
        (51..=100, 50, 0, -1) => (101, pos_x - 50, 1, 0),
        (101..=150, 0, 0, -1) => (151 - pos_x, 51, 0, 1),
        (151..=200, 0, 0, -1) => (1, pos_x - 100, 1, 0),
        (100, 1..=50, -1, 0) => (pos_y + 50, 51, 0, 1),
        (0, 51..=100, -1, 0) => (pos_y + 100, 1, 0, 1),
        (0, 101..=150, -1, 0) => (200, pos_y - 100, -1, 0),
        _ => panic!("shouldn't wrap with pos {}, {} and direction {}, {}", pos_x, pos_y, step_x, step_y)
    }
}

fn get_index_wrap(current_x: usize, current_y: usize, inc_x: i32, inc_y: i32) -> (usize, usize) {
    (
        (current_x as i32 + inc_x) as usize,
        (current_y as i32 + inc_y) as usize,
    )
}

fn parse(data: String) -> (Vec<Vec<char>>, Vec<Instruction>) {
    let mut parts = data.split("\n\n");
    let map = parts.next().unwrap();
    let path = parts.next().unwrap();
    let parsed_map = map.split("\n").map(|line| line.chars().collect()).collect();
    let mut number = 0;
    let mut parsed_path = Vec::new();
    for c in path.chars() {
        match c {
            'L' => {parsed_path.push(Instruction::Move { d: number}); number = 0; parsed_path.push(Instruction::Left);},
            'R' => {parsed_path.push(Instruction::Move { d: number}); number = 0; parsed_path.push(Instruction::Right);},
            d => number = number * 10 + d.to_digit(10).unwrap(),
        }
    }
    
    parsed_path.push(Instruction::Move { d: number});
    
    (parsed_map, parsed_path)
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn example1() {
        let result = solve1(input_example());
        assert_eq!(result, 6032);
    }

    #[test]
    fn exercise1() {
        let result = solve1(input_exercise());
        assert_eq!(result, 13566);
    }

    #[test]
    fn get_direction_test() {
        assert_eq!(get_direction(0, 1), 0);
        assert_eq!(get_direction(1, 0), 1);
        assert_eq!(get_direction(0, -1), 2);
        assert_eq!(get_direction(-1, 0), 3);    
    }
    
    #[test]
    fn example2() {
        let result = solve2(input_example());
        assert_eq!(result, 5031);
    }

    #[test]
    fn exercise2() {
        let result = solve2(input_exercise());
        // < 105298
        // < 45415
        assert_eq!(result, 11451);
    }

    fn input_example() -> String {
        "        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5".to_string()
    }


    fn input_exercise() -> String {
        "                                                  ....#.......#.........#......#...............#...##......................#......#..........#....##..
                                                  #.##.......#....#..#..................................#..................#..................#.......
                                                  ........#................................#.......#.#...................#...#....#........#....#.....
                                                  .........#.........##...........................##.......................#.....................#....
                                                  ....#..#..#............#....#........#.#.#.....................#................#....#....#.........
                                                  .............#.#.#..#.....#....#...............#.....#.......#.......#...........#..................
                                                  ............#...#..#.....#......#..#.......#........##.#..............#.............................
                                                  .................#.......#..#..........#......#....................................#.....#..........
                                                  ......#..........#.......#.......#......#........#.................#.#....#...###..........#........
                                                  ...#............................#.....#...................#..#.........#..............#....#........
                                                  ...#..#..............#..................#.....#....#...............#.#....###.......#...#...#.......
                                                  #...................#...#...........#....#..............#.......###.........#.............#.........
                                                  .......#...........##......#.............#.........#.....................................#.......#.#
                                                  ....#............#...............#...#...#.....#.#......#.....#..#.....##...................#.....#.
                                                  ...............##.........#..#.#............................................#........#...#..........
                                                  #...#...#.............#....#...##.........#.##........#......................#..........#......##..#
                                                  ...........................#..............##............................#...........................
                                                  .#........#........#.#.#..............#......................#........#.........#...................
                                                  ...#.#...........#......#....................#...........##...........................#.............
                                                  ...#..............#...#........###.##.......#.........................##.............#.#............
                                                  .......#...#........#..........#........#.....#..................................................#..
                                                  ................#.....##...........................###....#......#...#..............................
                                                  ...........................#....................#.................................#...#.............
                                                  .............#.....................#.#.........#...#.....#.......#..................................
                                                  .#...#..................#....#...........#..#..#.#.....#......................#.....................
                                                  #.........#...........................#...........#........#..................#..#.#.......####.##..
                                                  ...#..#.....#..#......#...#..........#........#.......#..........#......#......#.........#..........
                                                  .......#........#..........#.....#...#.#..#..........#.......#....#..#..#.....#.#.#....#..........##
                                                  ..##.................#.#...#....#.##.......#...........#..........#..#........#..#..#.#.....#.#.....
                                                  #.#.........................#.......#...........................#....................##....##..#....
                                                  ..................#....##................#......#..#.............#..................................
                                                  ...#.............##.....#.......................#...#............#..#....#..#........##....#........
                                                  ..........#.............#.......................#.....................##............................
                                                  #...#....#..........##............#..........###...#.............#.#........#..........#....#.......
                                                  ..#.........................#......#.....#..................##....##.............................#..
                                                  ............#.................#.................#..............#....#.......#..................#....
                                                  .............................#.....#.#.#...##..#...................#.................#.........#....
                                                  ...##.....#.#.##......#.#.....................#.....#..............#.#......#...................#...
                                                  ........................................................#.#..........................##..#...#......
                                                  ...................................#.........#.......#.......#.#...................#..#........#....
                                                  .............................#.....#.....#.................#......#...#.....#..##...............#...
                                                  .#..........#.....#..............#............#.........#.#......#.........#.................#......
                                                  ......#.#..#.#.....#....#............#..........#........................#....##........#.........#.
                                                  ....#............#......#...............#...........#.............#......................##.#...#...
                                                  .........#.......#..#........................#.......##.#........#......#...............#.......#...
                                                  .........#..............#....#...#............................#..#..........#.....#....#............
                                                  .#.........#..........#............................#..................#..........................#..
                                                  ........#.#.......#...........#........#.............................#.....#.#.#..............#..#..
                                                  ...................................#.........#........#...#.......#..............#.........#....#...
                                                  .#........#......................#.........................#......#.......#...........#....#........
                                                  ....#...#..............##...........#......#......
                                                  ..............#..........#........................
                                                  ...............#....#..#.............#..#..#.#....
                                                  .#.............#........................#.........
                                                  #....................#.......#..#...............##
                                                  ..#...............#....#...................###....
                                                  ............#.....#.........#...........#....##...
                                                  ......#.#...#..#.......#..........................
                                                  ................................#....#............
                                                  ............#......................#.#.......#.#..
                                                  ......##......#..#.........#........#.........#.#.
                                                  ..............#..............#.#.#.#...........#..
                                                  ...............#..#..............................#
                                                  .....................#................#.........#.
                                                  ..........#..............................#.......#
                                                  .........##......#..#....................#........
                                                  .....#..#................#.......#..........##....
                                                  .........#........#............#...#.#............
                                                  #...#.....#.......#...............................
                                                  ............................##...........##.......
                                                  ............#...................................#.
                                                  .............#..#.........#...........#...........
                                                  ...............##.........#.........#.....#.......
                                                  .............#.......#......#.........#...........
                                                  ..##.......#..#...#.................##............
                                                  ......#...#.......................................
                                                  .#....#...........#......#.....#........#.........
                                                  .#..#...........#......#...#......................
                                                  ..........#..................#.............#......
                                                  .....#.#..........##..................#....#......
                                                  ...............#...#...............#..............
                                                  ...........#....#...#.#..#..#.......#.............
                                                  .....#..............#..............#........#.....
                                                  ...................#.........................#.#..
                                                  ..........##.............#.....................##.
                                                  .............#.......#.........#.......##.........
                                                  .....#..#....#......#.##.....#...........#........
                                                  ...........#........#....#.....#.....#.#..#.......
                                                  ...#............#...#.#..#.........#........#.....
                                                  #.......#.........................................
                                                  .........##.#....#.................#..#.........#.
                                                  ..#....................................##.........
                                                  .#......#........#...................#.#..........
                                                  ......................#..#.............#.#........
                                                  ......#...#...#...#............#.....##......#....
                                                  #..#.#........#..............#.#.....#........#...
                                                  .#.......#.................##...........#......##.
                                                  ......................#..#........................
                                                  ...#....#......#....#......#......##...........#..
                                                  ......#.....#.....#..#........#....#..............
....#...#...........#......................##...##........#.....#....#........................#...#.
............##...#............#...#...............................#.................................
.....#.................#................#.............#..................#..#...#......#.....#.#....
.#..............#...............#.#...............#.....#.#..............#.#.....#.........#......#.
.....#...##......#...#..............#................................##.#.#....#.#.......#...#......
...........##........#.....#........#..#........................##..........#......#.##........#..#.
...........#......#..#.....#.........................##....#.....#......#.............#.....#.......
.....................##....#..#......#...................#........#.#....#........#..#..............
#..........#...........#........................................#..............#.......##........#..
........#..#.........................#......#...#...#........#.....................##...............
..##.....#..............................................#...................#..........#.#..........
.........#.......##.......................#.........###....................##......#..............#.
..............#..............##.#..............#................#.....#.............................
#............#.........#.#..#.................#...............#........#.....................#......
...........#....#...........#....#............................#.................#.........#.........
...#................................................#.#.....#....#.........#.........#.....##.....#.
#..............#............#...#...#....#..#..............##......#.#...........#.#.........#......
................#.......#...............#.....#...........#.........#...............................
....#..#.........#..#...................#.##...##.............#.......#........#.................#..
...........#.................#...........##..............##.....................#...............#...
...#........................#.........#..#.......#.........#.................#..#..#................
..#...............#.......#.........#...............#....................#..........................
.#...............................#.#..........#................#............#...#.........#..#......
#..#.........#.....#.........#.........................................#....#..............#....#.#.
...............................##...........#...#................#......#...............#..#........
......................................##...##......#..........#.........#..#.............#.#........
...............#......#....#...................#....#.#................#......#...#...............#.
.................#........#..#...#...#.............#......##......#....#.#.....#...#................
..............................#.............................##...............................#......
..#............#...#....#.......#..#.................#...#......#..##..#.#..#.....#..........#......
#..............#........#....#..#.#.....##.......................##............................#....
....................#..#............#................................#...........#...#..............
................#.#..................................#.......#...#.....#...#........................
..........#..........#.......#....#.......#.#....................#....#.................#..........#
..............#.................#.................#.......#.......#......#..#...................#...
................#.#.........#.##...#....#......#.........#...................###.#.........#........
#..........#...........#.............#...#...........................#..##..#.......................
..........#.............##.......#.........#........#...#.#.......#...................#........#....
..#.........#...........................#.#.#.........#....................................#........
..#.........#............................#...........#...#...................#......................
....#.......#...#.........#....#.............................#......##..#........................#..
....#..........................#...............#...#.....#...#...................#............#.....
.....#....#........#.......#..#.#............#..#...........#..##.............#..........#..........
........#......#..#...........#.............................#..........#.....#....................##
#..............#..#...#................#................................#.#...................#.#...
..#...........................#.#...........#.#............#...................#....#....#...##.....
.................#.................#...............#.....#........#..........................#..##..
...............#........#.............#..............#..#.....#.....#........................#......
.............#......#.................#..............#...#........#...#...#................#......#.
.....#........#..#.....#............#....#.....#............#.......................................
......#.........#.......##..#............#....#...
...........#.........#....#....#.................#
.....#......##................#.##................
...#........#.#.....#.#..............#......#.....
......##.#.#...........#....#...#.......#......#.#
.#......##..##..........#.........#.............#.
.##....#....#....................#................
...#......##............#..........#...##..#......
..##......##..................#........#..........
...#........#...#.#..................#............
.......................#.......#..................
....................................#.............
.............#.....#.#...............#........#...
............#..#...#........................#.....
.................#...............#..#.............
..................................#.#.............
..#.............#.#........#......................
.#.....................#..........................
......##.#..........................#.............
..................#.................#....#.......#
...#....#..#..#...........................#....#.#
........#..................#.#....................
............................#.............#.#.....
#.....#...#.......#..#.......#....................
.......................................#..#.....#.
......#...........#................#....#.....#...
......#.........#.........#..#..............#....#
.....##.......#.#..................#..............
.#.#..#...#........#..............#........#....#.
.........#..............#.........................
..#......#........................#...#...#.......
.................#.....#.....#....................
.#.......#......#...#.......#.....###......#......
#............#.....#..............................
#.......#..........#.....................#......#.
....#......#...........................#...#......
....#....#.....................#.....#............
........#.....................#...........##......
#...........#......#.#..#....#..#.................
................#.................#...............
............#.........#.....#......#..#.........#.
.....#.....#.................#......#.............
............#...#......##................#........
.#......#...........#.......#.#........##.........
.......#.......................................#..
....#..............#.......#........#......#......
........#............#.....##..........#..........
........#...........#.............#...............
.........#.........#.......#......................
#...#.##..................................#.......

".to_string()
    }
}
